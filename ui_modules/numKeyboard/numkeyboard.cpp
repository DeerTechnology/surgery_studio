...
NumKeyboard::NumKeyboard(QDialog *parent) :
    QDialog(parent),
    ui(new Ui::NumKeyboard)
{
    ...
    //pushButton_minus
    connect(ui->pushButton_minus,&QPushButton::clicked,this,&NumKeyboard::slotpushButton_minus);
    ui->pushButton_minus->setHidden(1);
}
...
void NumKeyboard::key_allow_negative()
{
    ui->pushButton_minus->setHidden(0);
}
void NumKeyboard::key_set_only_int()
{
    ui->pushButton_dian->setHidden(1);
}
...
void NumKeyboard::SetLimit(QString reNow)
{
    QRegularExpression  rx(reNow);
    QValidator *Validator = new QRegularExpressionValidator(rx);
    ui->lineEdit->setValidator(Validator);
}
void NumKeyboard::SetLimitRange(float max, float min)
{
...
void NumKeyboard::checkNum()
{
    if(ifSetLimitRange)
    {
        if(ui->lineEdit->text().toFloat()>rangeUpper)
        {
            ui->lineEdit->setText(QString::number(rangeUpper));
            strContent = QString::number(rangeUpper);
        }
        else if(ui->lineEdit->text().toFloat()<rangeLower)
        {
            ui->lineEdit->setText(QString::number(rangeLower));
            strContent = QString::number(rangeUpper);
        }
...
void NumKeyboard::slotpushButton_sure()
{
    ...
    emit signalNumKeyboardSure(strContent);
    this->close();
}
...
void NumKeyboard::slotpushButton_minus()
{
    int idx = ui->lineEdit->cursorPosition();
