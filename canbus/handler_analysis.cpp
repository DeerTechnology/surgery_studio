...
handler_analysis::handler_analysis(QObject *parent) : QObject(parent)
{

}

QByteArray handler_analysis::send_data_init(void *dataInfos, GlobalDataType type)
{
    QList<HandlerDataInfo> * datainfos = static_cast<QList<HandlerDataInfo> *>(dataInfos);
    //命令开头
    QByteArray res = "FEFE";
    switch (type)
    {
    ...
    float_uint f_to_i;
    foreach(HandlerDataInfo data, *datainfos)
    {
...
QList<HandlerDataInfo> handler_analysis::recv_data_init(QByteArray array)
{
    QList<HandlerDataInfo> infos;
    int num = array.at(1);
    int len = 0;
    for (int i = 0; i < num; i++)
    {
        HandlerDataInfo dataInfo;
        dataInfo.name = HandlerDataName(array.at(2 + i));

        switch (dataInfo.name)
        {
            case CantrolCMD_HANDLER://
